...
const Column = ({ title, links }: { title: string; links: { href: string; label: string }[] }) => <div><h2>{title}</h2>{links.map((x) => <Link key={x.href} href={x.href}>{x.label}</Link>)}</div>;
const MobileColumn = ({ title, links }: { title: string; links: { href: string; label: string }[] }) => <details><summary>{title}</summary><div>{links.map((x) => <Link key={x.href} href={x.href}>{x.label}</Link>)}</div></details>;
