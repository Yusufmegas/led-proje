...
export function ContentPage({ page }: { page: SeoPage }) {
  ...
  return <>
    <header className={`page-hero page-hero-${page.kind} ${heroImage ? "" : "page-hero-text-only"}`}><div className="container page-hero-grid"><div><Breadcrumbs current={page.h1} slug={page.slug} /><span className="eyebrow">{page.eyebrow}</span>{isPitch && <span className="pitch-watermark" aria-hidden="true">{page.slug.split("-")[0].toUpperCase()}</span>}<h1>{page.h1}</h1><p className="lead">{page.intro}</p><div className="button-row"><Link className="button" href="/iletisim#teklif">Projenize Özel Teklif Al</Link><a className="button button-outline" href={site.phoneHref}>{site.phoneDisplay}</a></div></div>{heroImage && <div className="page-hero-image"><Image src={heroImage} alt={`${page.h1} çözümünü kullanım ortamında gösteren LED ekran`} fill priority sizes="(max-width: 900px) 100vw, 42vw" /></div>}</div><div className="container page-proof-strip"><div><b>01</b><span>Teknik keşif</span></div><div><b>02</b><span>Sistem entegrasyonu</span></div><div><b>03</b><span>Montaj ve devreye alma</span></div><div><b>04</b><span>Teknik servis</span></div></div></header>
