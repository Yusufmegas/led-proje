...
export function SectorProjects({ sector }: { sector?: string }) {
  ...
  return <section className="sector-projects"><h2>Bu alandaki uygulamalar</h2><div>{projects.map((project) => {
    const card = <><div><Image src={assetPath(project.coverImage)} alt={project.title} fill loading="lazy" sizes="(max-width: 700px) 100vw, 50vw" /></div><h3>{project.title}</h3>{project.city && <p>{project.city}</p>}{project.summary && <p>{project.summary}</p>}</>;
