...
import { absoluteUrl, isNoindexDeployment } from "@/lib/site";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: page.title, description: page.description, alternates: { canonical: `/${slug}` }, robots: isNoindexDeployment ? { index: false, follow: false } : page.index ? { index: true, follow: true } : { index: false, follow: true }, openGraph: { title: `${page.title} | LEDProje`, description: page.description, url: absoluteUrl(`/${slug}`), type: "website" } };
}
